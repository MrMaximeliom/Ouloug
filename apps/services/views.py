...
class ServicesListView(BaseListView):
    searchManObj = None
    search_result = ''
        ...
        if 'filter_value' in request.GET  and 'clear' not in request.GET:
            ...
            if field_name == 'none':
                ...
                if field_name == "country":
                    self.search_result = Service.objects.filter(
                    ...
                    if order_by_filed and order_by_filed != 'none':
                        ...
                        if ordering == "asc":
                            self.search_result = Service.objects.filter(filtered_object).order_by(order_by_filed)
                        ...
                        self.search_result = Service.objects.filter(filtered_object)
