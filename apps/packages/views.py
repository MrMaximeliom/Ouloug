...
class PackagesListView(BaseListView):
    searchManObj = None
    search_result = ''
    ...
    def get(self, request, *args, **kwargs):
        ...
        from Util.lists_of_data import PACKAGE_STATUS
        ...
        if 'filter_value' in request.GET and 'clear' not in request.GET:
            ...
            if field_name == 'none':
                ...
                if field_name == "currency":
                    self.search_result = Package.objects.filter(
                        currency__country__name=field_value).order_by('id')
                    ordering = self.request.GET['filter_ordering']
                    order_by_filed = self.request.GET['order_by']
                    filtered_object = get_filtered_query(False, 'contains', field_name, field_value, 'country', True,
                                                         'name')
                    ...
                    if order_by_filed and order_by_filed != 'none':
                        ...
                        if ordering == "asc":
                            self.search_result = Package.objects.filter(filtered_object).order_by(order_by_filed)
                        ...
                        self.search_result = Package.objects.filter(filtered_object)
