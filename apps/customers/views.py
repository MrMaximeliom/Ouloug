...
class AgentShiftsListView(BaseListView):
    searchManObj = None
    search_result = ''
    ...
    def get(self, request, *args, **kwargs):
        ...
        if 'filter_value' in request.GET and 'clear' not in request.GET:
            ...
            if field_name == 'none':
                ...
                if field_name == "country":
                    self.search_result = AgentShift.objects.filter(
                    ...
                    if order_by_filed and order_by_filed != 'none':
                        ...
                        if ordering == "asc":
                            self.search_result = AgentShift.objects.filter(filtered_object).order_by(order_by_filed)
                        ...
                        self.search_result = AgentShift.objects.filter(filtered_object)
                ...
                if field_name == "team":
                    field_value = self.request.GET['filter_value']
                    self.search_result = AgentShift.objects.filter(
                        team__name=field_value).order_by('id')
                    ordering = self.request.GET['filter_ordering']
                    order_by_filed = self.request.GET['order_by']
                    filtered_object = get_filtered_query(True, 'contains', field_name, field_value, 'name')
                    if order_by_filed and order_by_filed != 'none':
                    ...
                    self.searchManObj.setPaginator(self.search_result)
                    self.searchManObj.setSearchPhrase(field_value)
                    self.searchManObj.setSearchOption('Team')
                    self.searchManObj.setSearchError(False)
                    self.searchManObj.set_querySet(self.search_result)
                    self.searchManObj.setSearch(True)
        ...
        self.extra_context = {
            ...
            'data_js': {
                ...
                "status": AGENT_CHOICES,
                "teams_list": list(Team.objects.values("name"))
            }
        }
...
class CustomersListView(BaseListView):
    ...
    def get(self, request, *args, **kwargs):
        from apps.address.models import Country, City, State
        from apps.customers.models import Customer, BusinessType
        from Util.lists_of_data import CUSTOMER_ACCOUNT_STATUS, CUSTOMER_PURCHASE_STATUS
        queryset = self.searchManObj.get_queryset()
        paginator = Paginator(queryset, 5)
        if 'filter_value' in request.GET and 'clear' not in request.GET:
            field_name = self.request.GET['filter_by']
            ...
            if field_name == 'none':
                ...
                if field_name == "country":
                    self.search_result = Customer.objects.filter(
                        city__state__country__name=field_value).order_by('id')
                    ordering = self.request.GET['filter_ordering']
                    order_by_filed = self.request.GET['order_by']
                    filtered_object = get_filtered_query(False, 'icontains', field_name, field_value, 'name',
                                                         value=field_value, args=('city', 'state', 'country'))
                    print("filtered objects are: ", filtered_object)
                    if order_by_filed and order_by_filed != 'none':
                        self.is_search_ordered = True
                        self.order_field = order_by_filed
                        if ordering == "asc":
                            self.search_result = Customer.objects.filter(filtered_object).order_by(order_by_filed)
                            self.ordering_value = "Ascending"
                        else:
                            order_by_filed = '-' + order_by_filed
                            self.search_result = Customer.objects.filter(filtered_object).order_by(order_by_filed)
                        ...
                        self.is_search_ordered = False
                        self.search_result = Customer.objects.filter(filtered_object)
                    self.searchManObj.setPaginator(self.search_result)
                    self.searchManObj.setSearchPhrase(field_value)
                    self.searchManObj.setSearchOption('Country')
                    self.searchManObj.setSearchError(False)
                ...
                if field_name == "state":
                    self.search_result = Customer.objects.filter(
                        city__state__name=field_value).order_by('id')
                    ordering = self.request.GET['filter_ordering']
                    order_by_filed = self.request.GET['order_by']
                    filtered_object = get_filtered_query(False, 'icontains', field_name, field_value, 'name',
                                                         value=field_value, args=('city', 'state'))
                    print("filtered objects are: ", filtered_object)
                    if order_by_filed and order_by_filed != 'none':
                        self.is_search_ordered = True
                        self.order_field = order_by_filed
                ...
                if field_name == "city":
                    self.search_result = Customer.objects.filter(
                        city__name=field_value).order_by('id')
                    ordering = self.request.GET['filter_ordering']
                    order_by_filed = self.request.GET['order_by']
                    filtered_object = get_filtered_query(False, 'icontains', field_name, field_value, 'name',
                                                         value=field_value, args=('city'))
                    print("filtered objects are: ", filtered_object)
                    if order_by_filed and order_by_filed != 'none':
                        self.is_search_ordered = True
                        self.order_field = order_by_filed
                    ...
                    self.searchManObj.setPaginator(self.search_result)
                    self.searchManObj.setSearchPhrase(field_value)
                    self.searchManObj.setSearchOption('City')
                    self.searchManObj.setSearchError(False)
                    self.searchManObj.set_querySet(self.search_result)
                    self.searchManObj.setSearch(True)
                if field_name == "business_type":
                    field_value = self.request.GET['filter_value']
                    self.search_result = Customer.objects.filter(
                        business_type=field_value).order_by('id')
                    ordering = self.request.GET['filter_ordering']
                    order_by_filed = self.request.GET['order_by']
                    filtered_object = get_filtered_query(False, 'equal', field_name, field_value)
                    if order_by_filed and order_by_filed != 'none':
                    ...
                    self.searchManObj.setPaginator(self.search_result)
                    self.searchManObj.setSearchPhrase(field_value)
                    self.searchManObj.setSearchOption('Business Type')
                    self.searchManObj.setSearchError(False)
                    self.searchManObj.set_querySet(self.search_result)
                    self.searchManObj.setSearch(True)
                if field_name == "account_status":
                    field_value = self.request.GET['filter_value']
                    self.search_result = Customer.objects.filter(
                        account_status__icontains=field_value).order_by('id')
                    ordering = self.request.GET['filter_ordering']
                    order_by_filed = self.request.GET['order_by']
                    filtered_object = get_filtered_query(False, 'equal', field_name, field_value)
                    if order_by_filed and order_by_filed != 'none':
                    ...
                    self.searchManObj.setPaginator(self.search_result)
                    self.searchManObj.setSearchPhrase(field_value)
                    self.searchManObj.setSearchOption('Account Status')
                    self.searchManObj.setSearchError(False)
                    self.searchManObj.set_querySet(self.search_result)
                    self.searchManObj.setSearch(True)
                if field_name == "purchase_status":
                    field_value = self.request.GET['filter_value']
                    self.search_result = Customer.objects.filter(
                        purchase_status__icontains=field_value).order_by('id')
                    ordering = self.request.GET['filter_ordering']
                    order_by_filed = self.request.GET['order_by']
                    filtered_object = get_filtered_query(False, 'equal', field_name, field_value)
                    if order_by_filed and order_by_filed != 'none':
                    ...
                    self.searchManObj.setPaginator(self.search_result)
                    self.searchManObj.setSearchPhrase(field_value)
                    self.searchManObj.setSearchOption('Purchase Status')
                    self.searchManObj.setSearchError(False)
                    self.searchManObj.set_querySet(self.search_result)
                    self.searchManObj.setSearch(True)
        if 'page' not in request.GET and 'filter_by' not in request.GET:
        ...
        self.extra_context = {
            ...
            "add_tool_tip_text": self.add_tool_tip_text,
            "update_tool_tip_text": self.update_tool_tip_text,
            "details_tool_tip_text":self.details_tool_tip_text,
            "instances_count": len(self.searchManObj.get_queryset()),
            "is_search_ordered": self.is_search_ordered,
            "order_by_field": self.order_field,
            "ordering_value": self.ordering_value,
            ...
            'data_js': {
                ...
                "cities_list": list(City.objects.values("name")),
                "business_types_list": list(BusinessType.objects.values("type_name")),
                "account_status": CUSTOMER_ACCOUNT_STATUS,
                "purchase_status": CUSTOMER_PURCHASE_STATUS,
            }
        }
        return super().get(request)
