...
class TelcomsFormView(OulougGroupPermission, FormView):
    # specify template name used to add new telcoms
    template_name = 'templates/telcoms/add_telcoms.html'
    # specify the form used
    form_class = TelcomForm 
    # specify the page to return to after successfully adding new telcoms
    success_url = 'telcoms'
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f"Telcoms {name} Added Successfully")
...
"""
TelcomsListView:
This class is used to view all added telcoms in the system,
it allows only administrators and monitor users to  
access.
"""


class TelcomsListView(OulougGroupPermission, ListView):
    # specify the model used in the view
    model = TelecomOperator
    # specify the template in the view
    template_name = "templates/telcoms/telcom_list.html"
...
class TelcomphoneFormView(OulougGroupPermission, FormView):
    # specify template name used to add new telcoms
    template_name = 'templates/telecom_phone/add_telecom_phone.html'
    # specify the form used
    form_class = TelecomphoneForm 
    # specify the page to return to after successfully adding new telcoms
    success_url = 'telecomphone'
    # specify user's groups allowed to access this view
    permission_required = ('administrator')

    # check if the form is valid or not after submitting it
    def post(self, request, *args, **kwargs):
        form = TelecomphoneForm(request.POST)
        if self.form_valid(form):
            print("valid")
        else:
            for field, items in form.errors.items():
    ...
    def form_valid(self, form):
        ...
        form.save()
        # present success message to the user
        messages.success(self.request, f"Telecomphone {name} Added Successfully")
        # return the control to the original overridden function in the super class
        return super().form_valid(form)
    ...
    extra_context = {
        ...
        'title': 'Add Telecomphone'
    }


"""
TelecomphoneListView:
This class is used to view all added telecomphone in the system,
it allows only administrators and monitor users to  
access.
"""


class TelecomphoneListView(OulougGroupPermission, ListView):
    # specify the model used in the view
    model = TelecomNumber
    # specify the template in the view
